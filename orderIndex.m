function [orderedIndex, orderedStims] = orderIndex(index, stimsFull)
...
for  type = 1:length(stimTypes)
    ...
    for j = 1:length(typeIndex)
        ...
        if exist('orderedStims', 'var')
            orderedStims = [orderedStims ; stimsFull(trialIndex,:)];
        else
            orderedStims = stimsFull(trialIndex,:);
        end
    end
