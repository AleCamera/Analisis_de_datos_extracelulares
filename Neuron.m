classdef Neuron
    ...
    methods
                ...
                if isfield(neu, 'Estimulos')
                    for s = 1:length(neu.Estimulos)
                        stimList(s) = struct('code', neu.Estimulos(s,1), ...
                            ...
                            'running', false,...
                            'reg', []);
                        if isfield(neu, 'EstReg')
                            stimList(s).reg = neu.EstReg{s};
                        end
                    end
        ...
        function [raster, index, stimList] = getRasters(obj, stimCodes, varargin)
            ...
            stimList = [obj.stims(stimIND).code];
        ...
        function [freq, tFrecs] = getPSHbyIndex(obj, xlimit, nBins, stimIndex, varargin)
            span = 5;
            method = 'moving';
            useSmooth = true;
            alphaGaussian = 2.5;
        ...
        function [t, ccg, trend] = getAutoCorr(obj)
            binSize = 1 / 1000;
            duration = 500 / 1000;
            span = 3;
            [ccg, t, ~, ~] = CCG([obj.data; obj.data], [ones(length(obj.data),1); 2*ones(length(obj.data),1)], 'BinSize', binSize, 'Duration', duration, 'mode', 'ccg');
            ccg = ccg(:,1,2);
            ccg(t == 0) = 0;
            trend = smooth(t(t>0),ccg(t>0),span);
            trend = [flip(trend);0; trend];
        end
