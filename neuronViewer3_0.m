function varargout = neuronViewer3_0(varargin)
% NEURONVIEWER3_0 MATLAB code for neuronViewer3_0.fig
%      NEURONVIEWER3_0, by itself, creates a new NEURONVIEWER3_0 or raises the existing
%      singleton*.
%
%      H = NEURONVIEWER3_0 returns the handle to a new NEURONVIEWER3_0 or the handle to
%      the existing singleton*.
%
%      NEURONVIEWER3_0('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NEURONVIEWER3_0.M with the given input arguments.
%
%      NEURONVIEWER3_0('Property','Value',...) creates a new NEURONVIEWER3_0 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before neuronViewer3_0_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to neuronViewer3_0_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help neuronViewer3_0

% Last Modified by GUIDE v2.5 23-Mar-2020 16:40:55

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @neuronViewer3_0_OpeningFcn, ...
                   'gui_OutputFcn',  @neuronViewer3_0_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before neuronViewer3_0 is made visible.
function neuronViewer3_0_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for neuronViewer3_0
handles.output = hObject;

%neuronViewer3_0 es una interfaz gráfica para visualizar y comparar la
...
%busco desde donde se corre el programa y lo cargo al config
fileFullName = mfilename('fullpath');
handles.programPath = fileFullName(1:end-16);
handles.settingsPath = [handles.programPath, slash, 'plottingParameters'];
handles.settings = load([handles.settingsPath, slash, 'plotSettings']);


% Update handles structure
guidata(hObject, handles);

% UIWAIT makes neuronViewer3_0 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = neuronViewer3_0_OutputFcn(hObject, eventdata, handles) 
...
%busco los indices de las neuronas a eliminar que tambien estan en la lista
%de seleccionadas
...
%genero el struct "par" que tiene los par�metros del experimento a plotear
%levanto los monitores
par.mDerecho = get(handles.mDerecho_check, 'Value');
par.mIzquierdo = get(handles.mIzquierdo_check, 'Value');

%levanto los tiempos
par.tPre = str2double(get(handles.tPre_edit, 'string'));
par.tPost = str2double(get(handles.tPost_edit, 'string'));

%Tambien cargo el tamaño de bins calculo el número de bins que necesito
par.binSize = str2double(get(handles.binSize_edit, 'string'));
par.nBins = round((par.tPre + par.tPost)*(1000/par.binSize));

%levanto la lista de estimulos
listStr = get(handles.stimCode_edit, 'string');
listCell = strsplit(listStr,' ');
for i = 1:length(listCell)
    par.stimCodes(i) = str2double(listCell{i});
end
...
[~, neuronIND,~] = intersect(neuronList, groupList,'stable');
neurons = {handles.neurons{neuronIND}};
...
if length(listCell) > 1
    ...
    elseif get(handles.compareNeurons_check, 'Value')
        ...
        plotPSHwithRasters(neurons, par, groupColor, vargs{:});
...
doubleAxis = false; %ESTA PROPIEDAD NO EST�? TERMINADA, NO ANDA BIEN.
...
function plotPSHwithRasters(neurons ,par, group, varargin)
    ...
    switch lower(varargin{arg})
        ...
        case 'relativesize'
            ...
            if relativeSize >= 1 || relativeSize <= 0
                error('relativeSize solo puede tomar valores entre 0 y 1')
            end
        case 'pshcolor'
...
while neuron <= nNeurons
    ...
    for nPlot = 1:plotsToDo
        %el nombre de la neurona
        name = neurons{neuron}.name;
        %obtengo los spikes del cluster seleccionado (index indica a que
        %trial corresponden)
        [raster,index, stims] = neurons{neuron}.getRasters(par.stimCodes, ...
            'Durations', [par.tPre, par.tPost], ...
            'Screens', [par.mDerecho, par.mIzquierdo]);
        %creo el titulo de la figura
        titulo = ['Neurona: ', name, '  Estimulos: ', num2str(par.stimCodes)];
        %Ploteo los rasters
        subplot(nRows, nCols, nPlot)
        hold on
        %armo el vector de frecuencias de disparo (Hz)
        [freq,~] = SyncHist(raster, index,'mode', 'mean' ,'durations',...
            [-par.tPre; par.tPost], 'nBins', par.nBins);
        if smoothPSH
            freq = smooth(freq, span);
        end
        
        vargs = {'PlotLines', useLinePlot,...
                 'Color', PSHface, ...
                 'StimHeigth', stimHeigth,...
                 'StimUnderPlot', stimUnderPlot, ...
                 'TopLimit', maxFreq};
        %armo el vector de tiempos
        t = -par.tPre:(par.tPre+par.tPost)/(par.nBins-1):par.tPost;
        %grafico el PSH
        hPlot = plotPSH(freq, t, stims,par.tPre, par.tPost, titulo, vargs{:});
        if ~useLinePlot
        ...
        if addStdError
            stdErr = getPSHstdErr(raster, index, par.nBins, par.tPre, par.tPost, ...
        ...
        PlotRasters_oneColor(raster, index, [-par.tPre, par.tPost],maxFreq, ...
